...
func ByName(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	for {
		...
		if err := reqBodyChecker(reqBody, REQ_KEY_ORG_CODE, REQ_KEY_DEPT_CODE, REQ_KEY_NAME, REQ_KEY_GTE, REQ_KEY_LT); nil != err {
			Logger.WriteLog("error", 10, "ES Http Request error :", err)
			res.SetResult("400", err.Error())
			break
		}

		cli_res, err := Call(reqBody, QUERY_FILE_BY_NAME, reqBody.Find(REQ_KEY_ORG_CODE))
		defer func() {
			cli_res.Body.Close()
			cli_res = nil
		}()
